    ...
    if (err) console.log(err);
      ...
      if (result == null) {
        ...
        collection.updateOne(
          query,
          { $set: { connect: socket.id, status: 1, login_status: "1" } },
          function (err) {
            if (err) console.log(err);
            //else
  ...
  collection.findOne(query, function (err, result) {
    if (err) console.log(err);
      ...
      if (result == null) {
        ...
        collection.updateOne(
          query,
          { $set: { connect: socket.id, status: 1, login_status: "0" } },
          function (err) {
            if (err) throw err;
            //else
            //console.log('- User socket_id:', socket.id);
  ...
  collection.findOne(query, function (err, result) {
    if (err) console.log(err);
      ...
      if (result == null) {
        ...
        if (userInfo.rank == "1") {
          if (userInfo.mapid == "1") add_exp = 5;
          else if (userInfo.mapid == "2") add_exp = 10;
        } else {
          add_exp = -2;
        }
